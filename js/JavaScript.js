...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const suprise = document.getElementById("suprise");// כפתור להפוך רקע לשחור 


  /* לכפתור COUNTER
  const countContactsBtn = document.getElementById("countContactsBtn");
   // פונקציה שמחשבת ומציגה את מספר אנשי הקשר הנוכחיים בספר הטלפונים
   function countContacts() {
     const count = contacts.length; // מחשב את מספר אנשי הקשר
     alert(`מספר אנשי הקשר בספר הטלפונים הוא: ${count}`); // מציג הודעת Alert עם המספר
   }
   // מאזין לאירוע לחיצה על הכפתור
   countContactsBtn.addEventListener("click", countContacts);
 */




  //  לפי סדר אלפבית הצגת אנשי הקשר
  ...
  function deleteContact(id) {
    ...
    const confirmDelete = confirm("האם אתה בטוח שברצונך למחוק את איש הקשר הזה?");
    
    // בדיקה אם המשתמש אישר את המחיקה
    if (confirmDelete) {`3E x
        contacts = contacts.filter(contact => contact.id !== id); // הסרת איש הקשר מהרשימה
        renderContacts(contacts); // הצגת רשימת אנשי הקשר המעודכנת
    } else {
        // אם המשתמש בחר לא למחוק, אפשר להוסיף פעולה כלשהי כאן אם תרצה (למשל הודעה אחרת או פעולה אחרת)
  ...
  //אירוע לחיצה על כפתור סופריז ,הופך את הרקע לשחור מקום כחול
